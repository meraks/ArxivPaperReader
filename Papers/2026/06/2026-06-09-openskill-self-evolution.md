...
> **传统范式 vs OpenSkill的类比**：
>
> 想象一个学徒进入一个新工坊学习木工技能：
>
> **Human-Curated范式**：师傅手把手教每个步骤，学徒只需模仿。问题：师傅只能教有限几种技能，遇到新家具类型就束手无策。
>
> **LLM-Generated范式**：学徒凭空想象怎么做，没有参考手册。问题：可能想出违反物理规律的方法（比如用胶水粘承重结构）。
>
> **Supervised Self-Evolution范式**：学徒反复尝试，师傅每次说"对"或"错"。问题：师傅必须在场监督，且师傅的判断可能出错。
>
> **OpenSkill范式**：学徒进入工坊的图书馆（开放世界），查阅木工手册、家具设计图、历史工匠的笔记（知识获取），然后根据手册上的标准尺寸和力学原理自己出题考自己（虚拟验证），确认无误后接受师傅的最终考核（零样本评估）。
>
> **关键洞察**：OpenSkill的突破在于——将"师傅的日常监督"替换为"图书馆的权威资料"，学徒可以完全自学，只要知道去哪里查、如何查、如何验证自己查对了。
