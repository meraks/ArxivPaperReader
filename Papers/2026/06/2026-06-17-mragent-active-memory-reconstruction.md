...
LLM 分析当前查询和推理上下文，从激活的 cues 出发，利用 $φ_{c\rightarrow g}$ 算子获得所有候选 tags。然后 LLM 根据语义相关性选择其中最相关的 tag 子集。例如，对于查询 "用户通常在什么时间喝咖啡？"，从 cue "咖啡" 激活的 tags 可能包括 "时间偏好"、"消费记录"、"口味偏好"，LLM 会优先选择 "时间偏好" tag。

**阶段 2：Content 访问**

基于选定的 tags，LLM 调用 $φ_{(c,g) \rightarrow v}$ 算子检索具体的内容节点。由于 tag 已经过第一轮筛选，返回的 contents 具有更高的语义针对性。继续上述例子，通过 ("咖啡", "时间偏好") 检索会返回所有与咖啡消费时间相关的对话段落，而排除口味、价格等其他维度的信息。
...
事件层存储具体的、按时间线组织的对话事件记忆。每个事件节点 $e_i \in V_e$ 表示一个离散的对话单元，例如 "用户在第三轮对话中询问了航班延误政策"。事件层具有以下特性：
...
语义层存储稳定的、去上下文化的知识单元。每个语义节点 $s_i \in V_s$ 表示从多次对话中提炼出的通用知识，例如 "用户的家乡是上海"、"用户偏好素食"、"用户是高级会员"。语义层的特性包括：
...
当 $T ≥ 2$时，存在 d 使得 $d+1 ≤ T < 2^{d-1}$（例如 $d = 5, T = 6$）。此时主动策略可以在预算内精确求解，而被动策略必然失败。因此存在函数 $f \in H^{active}_{LM}(T)$ 但 $f \notin H^{passive}_{LM}(T)$，证明严格包含关系。
