...
形式化表达：
- Passive retrieval: $$\pi_p(x)$$ — 仅基于初始查询x选择记忆单元
- Active reconstruction: $$\pi_a^{(t)}(x, S(t-1))$$ — 基于累积证据S(t-1)动态选择第t步动作

论文提供理论证明（Theorem 4.1）：对于任意检索预算T≥2，主动检索策略的假设类严格包含被动检索策略的假设类，即 $$H^{passive}_{LM}(T) \subsetneq H^{active}_{LM}(T)$$。

#### 2. Cue-Tag-Content异构图记忆

记忆被组织为异构图 $$M = (C, V, R)$$：
- **Cues (C)**: 细粒度关键词（实体、属性、时间戳等）
- **Contents (V)**: 具体记忆单元（对话片段、事件记录）
- **Tags (R ⊆ C × G × V)**: 带类型的关联边，将cues连接到contents
...
以上方法均为**被动检索**（passive retrieval）：检索策略$$\pi_p(x)$$仅基于初始查询x选择记忆单元，一次决定所有检索项，后续推理过程无法影响记忆访问路径。形式化表达：
...
第t步检索决策基于初始查询x和累积证据S(t-1)，每一步都依赖前序步骤发现的线索。
...
MRAgent 的核心记忆结构是一个异构图 (heterogeneous graph)，形式化为 M = (C, V, R)，其中：

- **Cues (C)**：细粒度的关键词索引单元，包括实体、属性、时间戳、描述符等。例如在对话历史中，cues 可能是 "咖啡"、"周五"、"下午3点"、"喜欢" 等词汇。这些 cues 作为记忆检索的入口点，它们本身不包含完整的语义信息，而是指向特定内容的索引标记。

- **Contents (V)**：具体的记忆项目，即实际需要存储和检索的信息单元。在对话场景中，content 可能是完整的对话段落 "用户说周五下午3点想喝咖啡"。Contents 是记忆系统的原子存储单元，在检索时被整体访问。

- **Tags (R ⊆ C × G × V)**：类型化的三元组关系，编码 cues 与 contents 之间的关联。每个 tag 表示一个 cue 与一个 content 通过特定类型 g 建立的关联。类型 g 可以是 "实体关系"、"时间属性"、"偏好表达" 等语义标签。例如，tag ("咖啡", "prefer Friday afternoon3pm", "用户说周五下午3点想喝咖啡") 将 cue "咖啡" 通过时间偏好类型连接到具体内容。
...
LLM 分析当前查询和推理上下文，从激活的 cues 出发，利用 ϕ_{c→g} 算子获得所有候选 tags。然后 LLM 根据语义相关性选择其中最相关的 tag 子集。例如，对于查询 "用户通常在什么时间喝咖啡？"，从 cue "咖啡" 激活的 tags 可能包括 "时间偏好"、"消费记录"、"口味偏好"，LLM 会优先选择 "时间偏好" tag。

**阶段 2：Content 访问**

基于选定的 tags，LLM 调用 ϕ_{(c,g)→v} 算子检索具体的内容节点。由于 tag 已经过第一轮筛选，返回的 contents 具有更高的语义针对性。继续上述例子，通过 ("咖啡", "时间偏好") 检索会返回所有与咖啡消费时间相关的对话段落，而排除口味、价格等其他维度的信息。
...
- **时序组织**：事件按时间戳排序，支持时间约束的查询（例如 "用户上周二说了什么？"）。时序信息通过时间戳 cues 编码在图中，agent 可以利用 ϕ 算子执行时间范围的过滤。
...
三个记忆层通过专门的 ϕ 算子连接，形成层次化的检索网络：

**ϕ_{τ→e}(τ)**：从主题节点下沉到相关事件节点。给定一个主题（例如 "货运咨询"），该算子返回所有属于该主题的事件。这支持 top-down 推理：agent 先识别问题主题，再访问相关具体事件。

**ϕ_{e→τ}(e)**：从事件节点上升到相关主题节点。给定一个事件，该算子返回该事件所属的所有主题。这支持 bottom-up 抽象：当新事件出现时，agent 可以识别其主题并将其连接到现有主题节点。

**ϕ_{e→s}(e)**：从事件节点提炼到语义节点。给定一个事件，该算子检查是否包含稳定的个人属性或偏好，如果发现则更新或创建对应的语义节点。例如，从事件 "用户说 '我下周要去上海'" 中提炼 "用户所在地：上海" 语义知识。

**ϕ_{s→e}(s)**：从语义节点定位到支持性事件。给定一个语义知识（例如 "用户偏好素食"），该算子返回所有支持该知识的事件证据。这支持知识溯源：当 agent 检索语义知识时，可以同时访问原始事件以提供上下文或验证。
...
- **主题连接**：新创建的主题节点被连接到所有支持它的事件节点，通过 ϕ_{τ→e} 和 ϕ_{e→τ} 算子实现双向访问。
...
重建过程在时刻 t 的状态定义为 S(t) = (Z(t), H(t))：

- **Z(t)**：当前激活的记忆元素集合，包括所有已被访问并保留的 cues、tags 和 content 节点。Z(t) 是记忆图的子图，表示 agent 在时刻 t 的 "工作记忆"。初始状态 Z(0) 包含从查询直接提取的 cues 匹配的存储 cues。

- **H(t)**：已累积的推理上下文 (evidence)，即从已访问内容中提取的与查询相关的信息片段。H(t) 是一个文本序列，由 LLM 从 Z(t) 中的 contents 中提炼出来，用于回答查询。初始状态 H(0) 通常为空或仅包含查询本身。

状态 S(t) 的二元结构区分了 "原始记忆数据" (Z(t)) 和 "推理结果" (H(t))。这种区分使 agent 能够在推理过程中灵活选择：是直接从原始内容中提取新证据，还是基于已有推理结果进行下一步动作。

### 4.1.2 遍历动作集

agent 可以执行的记忆遍历动作集合定义为 A = {Π_1, ..., Π_m}。这些动作分为前向遍历和反向遍历两大类：

**前向遍历动作**（从索引到内容）：
- **Π_{c→g}**：从 cues 激活 tags。给定 Z(t) 中的 cues，应用 ϕ_{c→g} 算子获取所有关联的 tags。例如，从 cue "咖啡" 激活 tags "时间偏好"、"消费记录" 等。
- **Π_{(c,g)→v}**：从 cue+tag 检索 contents。给定为 cue 和选定的 tag，应用 ϕ_{(c,g)→v} 算子获取具体的内容节点。例如，从 ("咖啡", "时间偏好") 检索所有与咖啡消费时间相关的对话段落。

前向遍历对应传统的记忆检索流程，从索引逐渐定位到具体内容。

**反向遍历动作**（从内容发现新索引）：
- **Π_{v→(c,g)}**：从已检索的 content 反向提取新的 cues 和 tags。给定 Z(t) 中的 contents，分析其文本内容，提取其中包含的 cues 和 tags，并将它们加入 Z(t+1)。例如，从检索到的对话段落中发现新的实体、时间或属性描述符。

反向遍历是 MRAgent 的关键创新，它使 agent 能够在检索过程中发现新的查询线索，扩展检索路径。这模拟了人类记忆检索中的联想过程：访问一个记忆会触发对相关记忆的联想。

### 4.1.3 组合动作空间

agent 在每个决策步骤可以选择执行动作的任意子集 A(t) ⊆ A。这种组合动作空间使 agent 能够：
...
给定当前状态 S(t) = (Z(t), H(t))，LLM 执行推理以选择下一步动作集合 A(t)：

- **输入**：LLM 接收三个输入：(1) 原始查询 q，(2) 已累积的推理上下文 H(t)，(3) 当前激活记忆元素 Z(t) 的描述（包括激活的 cues、tags 和已访问内容的摘要）。
  
- **推理目标**：LLM 判断当前 H(t) 是否足以回答 q。如果不足，则决定需要检索哪些新信息。这种决策基于当前的推理缺口：例如，如果 H(t) 包含用户的咖啡消费时间，但缺少频率信息，LLM 会决定检索与 "频率" 相关的内容。
  
- **动作选择**：LLM 从动作集 A 中选择子集 A(t)。选择可以是显式的（通过工具调用 API）或隐式的（通过解析 LLM 输出中的动作指令）。例如，LLM 可能决定同时执行 Π_{c→g}（从 cue "咖啡" 激活新 tags）和 Π_{(c,g)→v}（从现有 cue-tag 对检索内容）。

这一步骤将 LLM 的推理能力直接整合到记忆访问决策中，使检索策略能够根据推理上下文动态调整。

**步骤 2：受控记忆遍历**

对每个选中的动作 Π_a ∈ A(t)，系统执行对应的遍历算子，生成候选节点集合：

- **前向遍历**：如果 A(t) 包含 Π_{c→g}，系统对 Z(t) 中的每个 cue c 应用 ϕ_{c→g}(c)，获取所有关联 tags。如果包含 Π_{(c,g)→v}，系统对每个 (c, g) 对应用 ϕ_{(c,g)→v}(c, g)，获取候选 contents。
  
- **反向遍历**：如果 A(t) 包含 Π_{v→(c,g)}，系统对 Z(t) 中的每个 content v 分析其文本，提取其中包含的 cues 和 tags（通过 LLM 或规则提取器），并将这些新索引加入候选集合。
  
- **候选集合生成**：所有执行结果合并为 Z̃(t+1) = ∪_{a∈A(t)} Π_a(Z(t))。这是中间结果，尚未经过 LLM 路由。

这一步骤是记忆系统的机械操作，不涉及语义理解，确保了遍历的效率和可扩展性。

**步骤 3：LLM 路由与状态更新**

LLM 评估候选集合 Z̃(t+1) 中每个新节点的语义相关性，决定保留或丢弃：

- **相关性评估**：LLM 分析每个候选 content 与当前查询 q 和推理上下文 H(t) 的语义相关性。对于 content，LLM 判断其是否包含有助于回答 q 的新信息。对于新 cues 和 tags，LLM 判断它们是否可能导向相关内容。
  
- **路由决策**：基于相关性评估，LLM 生成过滤后的激活集合 Z(t+1) ⊆ Z̃(t+1) ∪ Z(t)，保留相关节点并丢弃无关节点。同时，LLM 从保留的 contents 中提取新的证据文本，更新推理上下文为 H(t+1)。
  
- **状态更新**：新状态 S(t+1) = (Z(t+1), H(t+1)) 成为下一轮迭代的输入。
...
- **充分性判定**：LLM 判断 H(t+1) 是否包含足够信息回答 q。如果是，则终止重建过程，输出 H(t+1) 作为最终记忆证据。
  
- **预算约束**：如果迭代次数达到预设上限 T_max（例如 8 轮），则强制终止，返回当前 H(t)。
  
- **收敛检测**：如果 Z(t+1) 相比 Z(t) 没有实质性变化（没有新的 relevant contents 被检索），则判定检索已收敛，终止过程。
...
- Z(0): 匹配的存储 cues + 直接连接的 tags
- H(0): 查询文本

**第一轮 (t=1)**：
- LLM 推理: 需要检索用户关于延误政策的具体陈述和情绪表达
- 动作选择: A(1) = {Π_{c→g}, Π_{(c,g)→v}}，从 cues 激活 tags 并检索内容
- 遍历结果: Z̃(1) 包含 10 个相关对话段落
- LLM 路由: 保留 3 个最相关的段落，提取用户抱怨改签费用高、赔偿不明确
- H(1): "用户在三次对话中提到改签费用过高，赔偿标准不透明"

**第二轮 (t=2)**：
- LLM 推理: 需要了解用户的具体经历（是否有实际延误经历）
- 动作选择: A(2) = {Π_{v→(c,g)}}，从已检索内容中发现新 cues
- 反向遍历: 从段落中发现新 cues {"改签", "费用", "赔偿", "4月3日航班"}
- LLM 路由: 激活新 cue "4月3日航班"，检索该具体航班事件
- H(2): 补充 "用户在4月3日航班延误后，被要求支付改签费用 500 元，未收到赔偿"

**第三轮 (t=3)**：
- LLM 推理: 需要确认用户的会员等级（是否影响赔偿政策）
- 动作选择: A(3) = {Π_{s→e}}，从语义层检索用户属性
- 遍历结果: 获取语义信息 "用户是 Gold 会员"
- LLM 路由: 检索 Gold 会员的赔偿政策条款
- H(3): 补充 "Gold 会员应享受免费改签和优先赔偿"

**终止 (t=4)**：
- LLM 判定: H(3) 包含完整信息（用户经历 + 政策标准 + 用户期望），可以回答查询
- 最终输出: H(3) 作为记忆证据传递给回答生成模块
...
被动检索策略 π_p: Q × M → V^T 将查询 q 和记忆图 M 映射到固定大小 T 的内容集合，且该选择仅依赖于 q，不依赖于任何中间状态。形式上：

π_p(q, M) = {v_1, ..., v_T}

其中 v_i ∈ V 是记忆图中的内容节点，T 是检索预算（允许访问的最大内容数）。被动检索的关键特性是 **one-shot 决策**：所有 T 个内容在观察到任何内容之前就已经被选定。
...
主动检索策略 π_a: Q × M × S(t) → A(t) 在每一步 t 基于当前状态 S(t) 选择动作 A(t)。形式上，主动检索生成一个访问序列：

v(1), v(2), ..., v(T)

其中 v(t) 的选择依赖于已访问内容 {v(1), ..., v(t-1)} 和累积的推理上下文 H(t-1)。主动检索的关键特性是 **sequential decision**：每一步的选择都可以利用之前步骤获得的信息。

主动检索策略类定义为：

H^{active}_{LM}(T) = {f: Q × M → Answer | ∃ π_a s.t. 在 T 步内以 f 方式访问 M}
...
H^{passive}_{LM}(T) ⊊ H^{active}_{LM}(T)
...
class CueTagContentGraph:
    def __init__(self):
        ...
        self.cue_to_tags = {}    # ϕ_{c→g} mapping
        self.tag_to_contents = {}  # ϕ_{(c,g)→v} mapping
