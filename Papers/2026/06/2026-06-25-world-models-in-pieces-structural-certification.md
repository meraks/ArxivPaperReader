...
**对比基线：**
1. **结构认证估计** (Structural Certification)：本文提出的算法
2. **经验频率估计** (Empirical Frequency)：直接从样本统计迁移频率
3. **均匀先验** (Uniform Prior)：对所有迁移分配相等概率
...
| 目标深度 n | 结构认证 MAE | 经验频率 MAE | 均匀先验 MAE |
|-----------|-------------|-------------|-------------|
| n = 5     | 基准水平    | 较高水平    | 最高水平    |
| n = 10    | 显著降低    | 中等降低    | 无变化      |
| n = 20    | 进一步降低   | 轻微降低    | 无变化      |
...
**1. 结构认证的样本效率优势**
- 在相同样本量 (N_samples = 4000) 下，结构认证比经验频率估计达到更低误差
...
**4. 均匀先验的局限性**
- 均匀先验的 MAE 不随 n 改善（无结构性知识）
- 在复杂环境中，均匀假设与真实动力学偏差显著
