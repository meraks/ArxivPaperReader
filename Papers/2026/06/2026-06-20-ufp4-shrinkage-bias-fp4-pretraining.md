...
- **E1M2（1 exponent, 2 mantissa）**：均匀网格（*文中称之为均匀网格，尽管其小数和大数部分的间隔并不一致，但其小数、大数部分分别保持一致*）
- **INT4**：均匀整数网格

>***补充知识***：
> E2M1的量化层级本身是IEEE 754浮点数值，详细的计算公式为：
> $$V = (-1)^{S} \times M \times E$$
> 其中$M$是尾数部分，$M=1+f$，而$f=2^{-1}b_{0}+2^{-2}b_{1}+\dots+2^{-n}b_{n-1}$，$b_{n}$为尾数的位数，$E$是指数部分，$E=2^{e-bias}$，$e$表示指数可以取到的值，$bias$的公式是$bias=2^{k-1}-1$。因此，完整的计算公式为：
> $$\text{V}=(-1)^{S} \times (1+2^{-1}b_{0}+2^{-2}b_{1}+\dots+2^{-n}b_{n-1}) \times 2^{e-bias}, bias=2^{k-1}-1$$
### 舍入策略
