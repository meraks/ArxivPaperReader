...
> **类比理解**：就像一支足球队，每个球员必须明确自己是守门员还是前锋。如果每个人都试图既守门又进攻，整个团队会陷入混乱。
...
> **类比理解**：机械可解释性是对神经网络做"逆向工程"——就像拆开一台机器，找出每个齿轮的功能，而不是仅仅观察输入输出。
...
> **类比理解**：位置推理像"第3排第5个座位"——剧场越大越容易找错；符号推理像"找穿红衣服的人"——无论剧场多大，只要看到红衣服就能定位。
...
> **类比理解**：RoPE像一个多针时钟——高频针（秒针）精确指示偏移量，低频针（时针）提供稳定性。当你需要恰好偏移d步时，只有特定位置的key能让所有时针对齐。就像用多个齿轮的咬合来判断步数——只有精确d步时所有齿轮同时归位。
...
> **类比理解**：位置机制像一个"步数计数器"——走100步和走1000步，计数器在900多步时已经飘了。符号机制像一个"路标识别器"——无论走多少步，只要路标清楚，就能认出来。
...
> **类比理解**：Number Task像爬楼梯——必须一层一层上（先学会1-hop，再学会2-hop...）；Letter Task像开灯——一旦掌握了"匹配"这个技能，所有hop数同时照亮。
...
> **类比理解**：就像人类认知——"第三排的第五个人"（位置记忆）在大会场里容易出错，但"穿蓝色衬衫的人"（特征记忆）始终准确。这不是记忆力的个体差异，而是两种认知策略的本质差异。
