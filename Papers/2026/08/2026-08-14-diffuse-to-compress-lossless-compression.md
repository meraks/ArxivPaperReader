...
**DLM 压缩率 vs 通用压缩器**：DLM 最差配置 CR=0.246 仍优于通用压缩器在 enwik8 上的表现 [VERIFY: 通用压缩器在 enwik8 上的具体 CR 值，研究材料未给出精确数字]，同时吞吐虽不及通用压缩器（差约三个数量级——通用压缩器比 LLM 方法高约六个数量级，DLM 比 LLM 方法高约 3.3 个数量级），但已远超 LLM 方法。
