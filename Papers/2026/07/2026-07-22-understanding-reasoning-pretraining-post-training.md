...
本文填补了预训练缩放定律（Kaplan 2020, Chinchilla 2022）和 RL 后训练缩放定律（Khatri 2025, Cheng 2026）之间的空白。预训练缩放律将下游使用视为给定，后训练缩放律将预训练初始化为固定。两者的共同空白是：预训练计算量如何改变 RL 缩放曲线的形状？本文直接从经验上建立了这一量化关系。

### 2.6 理论连接

Huang et al. (2025) 和 Chen et al. (2025) 证明覆盖度（coverage）可表征 Best-of-N 和 pass@k 性能，且下一 token 预测隐式优化覆盖度，从而连接预训练和后训练。本文从经验上补充了这一理论视角。
...
Chess 和数学两个领域的一致性结果揭示：预训练通过塑造损失景观（loss landscape）来影响 RL 后训练的效率。当预训练数据更多时，模型获得更好的初始表征，使 RL 梯度更新更高效。
