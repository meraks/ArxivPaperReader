...
3. **跨层路由机制（CLER / CLVR）**：提出轻量级线性时间跨层路径——CLER 将下层 delta-rule 写入误差注入上层；CLVR 将下层写入值注入共享隐藏流——验证了写入值而非写入误差是有效的跨层信号。
...
诊断发现 CLER 失败后，作者做了两项改动：①将路由目标从逐层值空间改为**共享隐藏流**（residual stream）；②将路由信号从写入误差改为**写入值**。
...
*图2：CLVR 将下层写入值（而非写入误差）注入共享隐藏流，经零初始化投影后加入残差流。此方案一致优于 CLER。*
...
**两个一致发现**：(1) 将路由目标从逐层值空间转移到对齐的隐藏流将比较从中性/负面转为小幅正面；(2) CLVR 一致优于 CLER-H，证明写入值而非写入误差是有效的跨层信号。因为 $$v_{l,t} = r_{l,t} + v̄_{l,t}$$，写入误差 = 写入值减去记忆自身的读取值，所以路由误差仍然携带下层记忆尚未吸收的值部分，但路由完整值效果更好。
...
2. **混合堆栈通常改善损失但降低部分速度优势**。最终的选择取决于目标场景——追求最低损失选 KDA+Muon 混合堆，追求最大吞吐量选纯 GDN+AdamW。
...
2. **CLVR 效果有限**：跨层值路由的增益较小（350M/15B 时 Δ ~ -0.002 到 -0.006），且随训练增加和模型增大递减。未在大规模 DeltaNet 上验证（1.3B/40B DeltaNet 隐藏流路由比较未报告）。
...
6. **未评估 Attention Residuals 在两种宿主间的一致性**：Attention Residuals 在 DeltaNet 上改善损失（-0.0101），在 GDN 上略增损失（+0.0030），效果不跨宿主迁移。
