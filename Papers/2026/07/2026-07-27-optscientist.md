...
| 优化器状态内存增量（相对 AdamW 基线） | 仅多 **7.45 MB** | — | 接近零成本 |
...
- **Reduced-State（精简状态）**：相较于 Muon 通常维护动量 + 正交化中间量，RS-MR **仅维护 2 个跨步状态变量**——一个动量 $m_t$ 与一个块级门控 $g_t$。状态的精简直接带来了内存优势（仅多 7.45 MB，见第 5 章）。
...
3. **内存开销极小**：RS-MR 相较 AdamW 基线仅多消耗 **7.45 MB** 优化器状态内存。考虑到 NanoChat d21 的模型参数本身约为 4.2 GB（FP32）/ 2.1 GB（FP16），7.45 MB 的增量**完全可以忽略**（< 0.4%）。这得益于 RS-MR 仅维护 2 个状态，且门控是标量级。
4. **Shampoo/Sophia 性价比低**：Shampoo 类方法虽然 BPB 尚可，但内存开销远高于 RS-MR，位于 Pareto 前沿的右上方（次优区域）；Sophia 则在两个维度上都劣势。
5. **AdamW 内存最优但性能最差**：AdamW 位于前沿的左上角（内存最优但性能最差），是"内存极限"的锚点，但绝非性能优选。

> **结论**：RS-MR 是当前**性能—内存权衡的最优点**，既没有 Shampoo 的内存负担，也没有 AdamW/Sophia 的性能短板，更严格支配了 Muon。这对实际部署有直接价值：在内存受限的场景下（如消费级 GPU 微调大模型），RS-MR 是比 Muon 更优的选择。
