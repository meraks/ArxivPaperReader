...
论文明确指出：LoRA 并非全量微调的近似，而是其**泛化形式**。当对所有权重矩阵应用 LoRA 且将秩 $r$ 设为预训练权重矩阵的秩时，LoRA 可恢复全量微调的全部表达能力。换句话说，随着可训练参数量增加，LoRA 训练大致收敛到原始模型训练；而 Adapter 方法收敛到一个 MLP，Prefix Tuning 方法收敛到一个无法处理长输入序列的模型。这一性质从理论上保证了 LoRA 在参数预算充足时不会劣于全量微调。

### 3.7 训练效率优势

冻结绝大部分参数带来了三方面的系统级收益：

- **训练加速约 25%**：冻结参数无需计算梯度，在 GPT-3 175B 上观察到训练吞吐从全量微调的 **32.5 tokens/s/V100** 提升至 LoRA 的 **43.1 tokens/s/V100**（约 32% 提升，论文保守记为 25%）。
...
- **检查点体积压缩约 10,000×**：从全量微调的约 **350GB** 检查点压缩到 LoRA 的约 **35MB**（以 r=4 仅适配 W_q、W_v 为例），便于存储、分发与版本管理。实际部署时，存储 100 个适配模型仅需 350GB（基座）+ 35MB×100 ≈ **354GB**，而全量微调需 350GB×100 ≈ **35TB**。
...
其中 4.7M 配置对应 `r_q=r_v=1` 或仅 `r_v=2`；37.7M 配置对应 `r_q=r_v=8` 或 `r_q=r_k=r_v=r_o=2`（即对所有四种投影矩阵施加 LoRA）。
...
3. **规模收益**：模型越大，LoRA 的相对优势越突出——大模型更接近"低秩适配"的理想区间。原文 Figure 2 展示了 GPT-3 上各方法的"参数量-性能"曲线，LoRA 在同等参数预算下始终位于最优点。
...
- **`lora_A`**：形状 $(r, \text{in\_features})$，使用 **Kaiming uniform** 初始化（保证前向方差稳定）。
- **`lora_B`**：形状 $(\text{out\_features}, r)$，**零初始化**——这保证训练初始时 $\Delta W = BA = 0$，即起步等价于预训练模型，**不破坏初始的预训练表征**。
- **缩放因子**：

$$
\text{scaling} = \frac{\text{lora\_alpha}}{r}
$$
...
| **理论位置** | LoRA 是全量微调的泛化（$r=\text{rank}(W)$ 时恢复全量表达力） |
