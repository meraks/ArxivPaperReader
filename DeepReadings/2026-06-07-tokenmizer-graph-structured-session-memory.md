...
方差本身是一种验证：高方差（32.7% σ for task recall）表明提取质量与领域特征强相关，这与 TokenMizer 的设计预期一致 —— 启发式提取器依赖于文本中的显式信号。
...
3. **Heuristic-only evaluation**：LLM 提取器已实现但未评估。对于隐式推理场景（如 Research 领域），LLM 提取器可能显著提升召回率，但也会引入延迟和成本。
4. **Decision CSV 拆分回归**：~1pp 的决策回忆率下降由 CSV 拆分引入，需要优化。
5. **非英语语言**：34 条正则基于英语动词模式，对其他语言未评估。
6. **语义缓存仅初步验证**：受控 10 查询，非生产级。
...
checkpoint:
  tier: standard           # critical | standard | full
  trigger_ratio: 0.85      # 触发检查点的 MECW 比例
  mecw: 16000              # Maximum Effective Context Window
extraction:
  engine: v2               # v1 | v2 | llm
  validator_threshold: 0.50
compression:
  heuristic: true
  neural: false            # 需要 LLMLingua-2
cache:
  enabled: true
...
**部署方式**：将 OpenAI 兼容客户端指向 `http://localhost:8000`：
- 无 `session_id` → 零开销透传
- 有 `session_id` → 自动图谱提取和检查点

支持的 LLM 提供商：Anthropic (Claude 3/4), OpenAI (GPT-4o, o-series), Google (Gemini 1.5/2.0, free tier available), Groq (Llama 3.x, free), OpenRouter (100+ models), DeepSeek, Mistral, Cohere, Ollama (local models, free)。

### 7.3 与相关工作的定位

| 系统 | 记忆形式 | 结构化 | 成本 | 代理模式 |
|------|---------|--------|------|---------|
| **TokenMizer** | 类型化知识图谱 | ✅ | 启发式 $0 / LLM 可选 | 透明代理 |
| MemGPT | 自然语言记忆块 | ❌ | LLM 推理 | OS 风格分页 |
| LangChain KG | 无类型图谱 | 部分 | 应用层集成 | 框架 |
| ACC (Anthropic) | 压缩文本 | ❌ | LLM 推理 (22.7% 压缩) | 端到端 |
| GraphRAG | 文档聚类图谱 | 部分 | 构建成本高 | 离线索引 |

### 7.4 延伸阅读

- **MECW**: Paulsen, 2025. "Maximum Effective Context Window in Large Language Models"
- **Lost in the Middle**: Liu et al., 2023. "Lost in the Middle: How Language Models Use Long Contexts"
- **MemGPT**: Packer et al., 2024. "MemGPT: Towards LLMs as Operating Systems"
- **LLMLingua-2**: Pan et al., 2024. "LLMLingua-2: Data Distillation for Efficient and Faithful Task-Agnostic Prompt Compression"
- **GraphRAG**: Edge et al., 2024. "From Local to Global: A Graph RAG Approach to Query-Focused Summarization"
- **Agent Memory Survey**: [TsinghuaC3I/Awesome-Memory-for-Agents](https://github.com/TsinghuaC3I/Awesome-Memory-for-Agents)
- **ACC**: Anthropic, 2024. "Anthropic's Context Compression"

### 7.5 实用价值与未来方向

TokenMizer 填补了 AI Agent 基础设施的一个关键空白：**会话级结构化状态管理**。当前大多数 Agent 框架（LangChain, AutoGPT, CrewAI）依赖开发者手动管理状态或通过平面文本总结，TokenMizer 提供了一种「开箱即用」的自动化方案。

**未来方向**：
1. LLM 提取器的完整评估（对 Research/隐式推理场景可能显著改善）
2. 多标注者 ground-truth 和真实生产环境的 benchmark 扩展
3. 跨会话记忆（当前仅单会话，与 MemGPT 的桥接是自然延伸）
4. 增量图序列化的 diff 格式（目前 `graph_diff={}` 为空）
5. 多 Agent 场景下的共享知识图谱
...
```
Require: Graph G, budget Tbudget
Ensure: Resume block R

1:  R ← ["[CHECKPOINT: {session_id}]"]
2:  Tbudget ← Tbudget − count_tokens(R[0])
3:  S ← sort(nodes(G), key=importance, descending)
4:  for category in [COMPLETED_TASKS, PENDING_TASKS, DECISIONS, FILES, ENV]:
5:      for node in S where node matches category:
6:          line ← category_label + ": " + node.label
7:          if count_tokens(line) ≤ Tbudget:
8:              R.append(line)
9:              Tbudget ← Tbudget − count_tokens(line)
10:         else:
11:             break  # 预算耗尽，停止填充分类
12: return "\n".join(R)
```

### Algorithm 2: Fuzzy Label Matching (Definition 5)

```
Require: extracted label a, ground-truth label b
Ensure: match: bool

1:  a ← lower(a), b ← lower(b)
2:  if a ⊆ b or b ⊆ a: return True          # 子串包含
3:  Wa ← {w in tokens(a) where len(w) ≥ 3}  # ≥3 字符过滤
4:  Wb ← {w in tokens(b) where len(w) ≥ 3}
5:  if Wa = ∅ or Wb = ∅: return False
6:  shorter ← min(|Wa|, |Wb|)
7:  return |Wa ∩ Wb| / |shorter| ≥ 0.50     # 50% token 重叠
```

### Algorithm 3: Node Upsert with Confidence

```
Require: candidate node n, validator v
Ensure: Graph updated

1:  n.confidence ← v.score(n)
2:  if n.confidence < v.min_confidence: return   # 低置信度丢弃
3:  if n.id in G.nodes:
4:      existing ← G.nodes[n.id]
5:      existing.importance ← min(1.0, existing.importance + 0.1)
6:      if STATUS_ORDER[n.status] > STATUS_ORDER[existing.status]:
7:          existing.status ← n.status             # 单调升级
8:  else:
9:      G.nodes[n.id] ← n
10: if |G.nodes| > 500:
11:     prune(G)                                    # 驱逐 importance < 0.1
```

---

*报告完成于 2026-06-07 | 字数 ~6000 字 | 论文 Deep Read*