...
| 编号 | 内容 | 章节 |
|------|------|------|
| **Figure 1** | 后训练四阶段管线 + Strong-to-Weak Distillation | 第 5 章 |
| **Table 1** | Dense 模型架构配置（6 个模型） | 第 3 章 |
| **Table 2** | MoE 模型架构配置（2 个模型） | 第 3 章 |
| **Table 3** | Qwen3-235B-A22B Base vs 竞品（15 benchmarks × 5 models） | 第 6 章 |
| **Table 4** | Qwen3-32B Base vs 竞品 | 第 6 章 |
| **Table 5** | Qwen3-14B / 30B-A3B Base vs 竞品 | 第 6 章 |
| **Table 6** | Qwen3-8B Base vs 竞品 | 第 6 章 |
| **Table 7** | Qwen3-4B Base vs 竞品 | 第 6 章 |
| **Table 8** | 小模型 Base (0.6B/1.7B) vs 竞品 | 第 6 章 |

旗舰模型 Qwen3-235B-A22B 仅用 DeepSeek-V3 约 1/3 的总参数和 3/5 的激活参数，在 15 项 benchmark 中取得 14 项领先（详见第 6 章）。
...
Qwen3 的实验评估围绕一个核心命题展开：**用更少的参数，取得更强的性能**。评估体系覆盖 4 大类别共 15 个 benchmark，兼顾通用知识、数学与 STEM、代码生成、多语言能力，并对 base model（预训练模型）与 post-trained model（后训练模型）分别给出结果。

### 6.1 评估基准体系（4 类别，15 个 benchmark）
...
- **越级挑战**：在 MMLU-Pro（65.54 vs 58.07）、SuperGPQA（39.78 vs 36.20）、BBH（87.38 vs 86.30）、EvalPlus（72.05 vs 65.93）四项推理密集型 benchmark 上，**32B 的 Qwen3 超越自家上一代 72B 模型**（精度、数学等 5/15 项仍有差距，见通用知识说明）。这说明 Qwen3 在同等规模下的"单位参数效率"有质的飞跃。
...
#### 6.2.3 Qwen3-14B-Base & Qwen3-30B-A3B-Base（Table 5）

Qwen3-14B-Base 和 Qwen3-30B-A3B-Base（MoE 30B/3B）对比同级竞品及更大规模模型：

| Benchmark/Model | Gemma-3-12B (Dense 12B) | Qwen2.5-14B (Dense 14B) | Qwen2.5-32B (Dense 32B) | Qwen2.5-Turbo (MoE 42B/6B) | **Qwen3-14B (Dense 14B)** | **Qwen3-30B-A3B (MoE 30B/3B)** |
|:---|:---:|:---:|:---:|:---:|:---:|:---:|
...
- **越级对比**：Qwen3-14B-Base 在 MMLU-Pro（61.03）、EvalPlus（72.23）、MATH（62.02）等推理密集任务上显著超越 Qwen2.5-32B-Base（55.10/66.25/57.70），验证了数据质量与训练策略带来的"单位参数效率"跃升。
- **MoE 效率优势**：Qwen3-30B-A3B 仅 3B 激活参数即可在 10/12 项 benchmark 上超越 Qwen2.5-14B-Base（14B 全激活），激活参数仅为后者的 ~1/5。在 MMLU-Pro、SuperGPQA、BBH 上甚至接近 Qwen2.5-32B-Base。
- **14B vs 30B-A3B 互补**：Qwen3-14B 在数学（MATH 62.02）和代码（EvalPlus 72.23）上更强；Qwen3-30B-A3B 在 MMLU（81.38）、BBH（81.54）等多任务通用基准上更优。两种架构各有侧重。

#### 6.2.4 Qwen3-8B-Base（Table 6）

| Benchmark/Model | Llama-3-8B (Dense 8B) | Qwen2.5-7B (Dense 7B) | Qwen2.5-14B (Dense 14B) | **Qwen3-8B (Dense 8B)** |
|:---|:---:|:---:|:---:|:---:|
...
- Qwen3-8B-Base 在 **12/12 项 benchmark 全面超越 Llama-3-8B-Base**，部分指标近乎翻倍（MATH 60.80 vs 20.50, GSM8K 89.84 vs 55.30），体现了两代模型间的数据规模与训练范式差距。
- **越级挑战**：Qwen3-8B（8B 参数）在 MMLU-Pro（56.73 vs 51.16）、GPQA（44.44 vs 32.83）、MATH（60.80 vs 55.64）、EvalPlus（67.65 vs 60.70）上超越更大规模的 Qwen2.5-14B-Base，尤其是 STEM 和推理任务提升显著。

#### 6.2.5 Qwen3-4B-Base（Table 7）

| Benchmark/Model | Gemma-3-4B (Dense 4B) | Qwen2.5-3B (Dense 3B) | Qwen2.5-7B (Dense 7B) | **Qwen3-4B (Dense 4B)** |
|:---|:---:|:---:|:---:|:---:|
...
- Qwen3-4B（仅 4B 参数）在 **12/12 项 benchmark 上全面超越同级 Gemma-3-4B**，优势在推理任务上尤其突出（MATH 54.10 vs 26.10, MMLU-Pro 50.58 vs 29.23）。
- **越级挑战**：MMLU-Pro（50.58）、GSM8K（87.79）、EvalPlus（63.53）等多项指标超越参数翻倍的 Qwen2.5-7B-Base。MATH 提升 4.3pp（54.10 vs 49.80），BBH 提升 2.19pp（72.59 vs 70.40）。
- 在 MMLU 和 MMMLU 上，Qwen3-4B 略低于 Qwen2.5-7B-Base，说明通用知识密集型任务仍受益于更大参数规模。

#### 6.2.6 Qwen3-0.6B / 1.7B-Base（Table 8）

| Benchmark/Model | Qwen2.5-0.5B | **Qwen3-0.6B** | Gemma-3-1B | Qwen2.5-1.5B | **Qwen3-1.7B** |
|:---|:---:|:---:|:---:|:---:|:---:|
...
- **端侧模型大幅领先**：Qwen3-0.6B 在几乎所有基准上超越 Qwen2.5-0.5B，MMLU-Pro（24.74 vs 15.69）和 BBH（41.47 vs 20.30）的翻倍式提升尤为显著。
- **Qwen3-1.7B 跨级碾压**：在 BBH（54.47 vs 45.10）、GSM8K（75.44 vs 68.54）、MATH（43.50 vs 35.00）、MultiPL-E（42.71 vs 33.10）等推理密集任务上超越参数相近的 Qwen2.5-1.5B。MMLU-Pro（36.76 vs 28.53）和 MGSM（50.71 vs 32.82）的大幅差距反映了 Qwen3 在多语言与深层推理上的系统性改进。
- **Gemma-3-1B 全线落后**：Qwen3-0.6B 以 1/2 的参数规模全面超越 Gemma-3-1B，GSM8K（59.59 vs 2.20）和 CRUX-O（27.00 vs 3.80）差距达 20-30 倍，说明 Qwen3 在小规模上的训练数据与策略具有决定性的优势。
