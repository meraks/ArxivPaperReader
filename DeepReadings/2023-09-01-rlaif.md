...
- **作者**: Harrison Lee等人 (Google DeepMind)
- **会议**: ICML 2024
- **arXiv**: 2309.00267
...
**数据格式**：D = {(x_i, y_i)}，其中x_i是输入，y_i是目标输出
...
**数据格式**：D = {(x, y_w, y_l)}，其中y_w是更优的响应，y_l是较差的响应
...
**PPO目标**：
$$
\mathcal{L}(\theta) = \mathbb{E}\left[ \min\left(\frac{\pi_\theta(y|x)}{\pi_{\theta_{\text{old}}}(y|x)} A(x,y), 1 + \epsilon\right) A(x,y) \right] - \beta \cdot \text{KL}[\pi_\theta || \pi_{\text{ref}}]
$$

其中A(x,y)是优势函数，ε是裁剪参数（典型值为0.2）
...
graph TB
    subgraph 正向推理
        ...
        A2 --> A3[P(A≻B)_forward]
    end
    
    subgraph 反向推理
        B1[Input: premise<br/>Response 1: B<br/>Response 2: A] --> B2[LLM推理]
        B2 --> B3[P(A≻B)_backward]
    end
    
    A3 --> C[最终偏好]
    B3 --> C
    
    C --> D[P(A≻B) = (P_forward + (1-P_backward))/2]
...
其中：
- P(A ≻ B)_forward是A在前、B在后的推理结果
- P(A ≻ B)_backward = 1 - P(B ≻ A)_backward是B在前、A在后时的转换结果
...
**1. 摘要任务 - Reddit TL;DR**
- 数据规模：123,000个Reddit帖子
- 标注数据：92,000个成对比较
- 下采样：3-4k训练示例
- 评估：与人类参考摘要和RLHF模型比较

**2. 有用对话 - Anthropic Helpful-Human Dialogue**
- 数据规模：>40,000训练示例
- 下采样：3-4k训练示例
- 任务：生成有帮助的对话响应
- 评估：有用性偏好比较

**3. 无害对话 - Anthropic Harmless-Human Dialogue**
- 数据规模：>40,000训练示例
- 下采样：3-4k训练示例
- 任务：生成安全的对话响应
- 评估：无害率（拒绝有害请求的比例）
